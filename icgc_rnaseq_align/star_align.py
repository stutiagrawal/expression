...
if __name__ == "__main__":
    ...
    star.add_argument("--scoreInsOpen", type=int, default=-2, help="scoreInsOpen")
    star.add_argument("--alignEndsType", type=str, default='Local', help="alignEndsType")
    star.add_argument("--quantMode", type=str, default='-', help="quantMode")
    star.add_argument("--sjdbGTFfile", default=None, help="path to annotation file")
...
--quantMode ${quantMode}""" % read_str
        ...
        cmd = string.Template(align_template_str_1st).safe_substitute({
            ...
            'alignIntronMin' : args.alignIntronMin,
            'scoreDelOpen' : args.scoreDelOpen,
            'scoreInsOpen' : args.scoreInsOpen,
            'alignEndsType' : args.alignEndsType,
...
--sjdbOverhang %i \
--runThreadN %i \
--limitGenomeGenerateRAM %i \
--sjdbFileChrStartEnd %s \
--sjdbGTFfile %s """ % (genome_dir_1st, args.genomeFastaFiles, args.sjdbOverhang, args.runThreadN, args.limitGenomeGenerateRAM, os.path.join(align_dir_1st, 'SJ.out.tab'), args.sjdbGTFfile)
        print "Running", cmd
        subprocess.check_call(cmd, shell=True, cwd=align_dir_1st)
